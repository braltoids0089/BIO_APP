# This is a patched version of the Streamlit digital twin therapy selection app.
# The original script lacked an import for the Counter class from the
# collections module, which resulted in a NameError at runtime when
# computing drug selection frequencies. This patched script adds the missing
# import to prevent that error.  Aside from the import, the logic is
# identical to the original file fetched from GitHub.
...
def generate_synthetic_expression(
    ...
    """Generate a synthetic gene expression matrix with lognormal values."""
...
def pathway_scores(expr_symbols: pd.DataFrame, signatures: dict) -> pd.DataFrame:
    """Calculate average z-scored expression per pathway per sample."""
...
def example_drug_panel() -> dict[str, list[str]]:
    """Define a simple drug panel mapping drugs to Reactome pathways."""
    return {
